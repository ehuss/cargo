use cargotest::support::{basic_lib_manifest, execs, lines_match, project, Project};
...
fn metabuild_metadata() {
    ...
    let src_path = meta["packages"]
        ...
        .unwrap()
        .iter()
        .filter(|t| t["name"].as_str().unwrap() == "metabuild-foo")
        .next()
        .expect("expected metabuild-foo target")["src_path"]
        .as_str()
        .expect("expected src_path");
    assert!(
        lines_match(
            "[..][/]foo[/]target[/].metabuild[/]metabuild-foo-[..].rs",
            src_path
        ),
        "{} did not match",
        src_path
