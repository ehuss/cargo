...
fn workspaces() -> Vec<(String, PathBuf)> {
    ...
    let mut ps: Vec<_> = match std::env::var_os("CARGO_BENCH_WORKSPACES") {
        ...
        None => fs::read_dir(workspaces_path())
            .unwrap()
            .map(|e| e.unwrap().path())
            // servo currently fails in most cases on Windows due to long
            // filenames in the git checkouts.
            .filter(|p| !(cfg!(windows) && p.file_name().unwrap() == "servo"))
            .collect(),
    };
...
/// download everything, and then passes the information to your callback to
/// run a benchmark.
///
/// This uses a callback just to avoid dealing with `Config` ownership.
/// Alternatively, this could take `Config` as an input, and return
/// `ResolveInfo`.
fn do_resolve(ws_root: &Path, f: &mut dyn FnMut(ResolveInfo)) {
...
fn resolve_ws(c: &mut Criterion) {
    ...
    for (ws_name, ws_root) in workspaces() {
        group.bench_function(&ws_name, |b| {
            ...
            do_resolve(&ws_root, &mut |ResolveInfo {
                                           ws,
                                           requested_kinds,
                                           target_data,
                                           cli_features,
    ...
    for (ws_name, ws_root) in workspaces() {
        group.bench_function(&ws_name, |b| {
            do_resolve(&ws_root, &mut |ResolveInfo {
                ...
                b.iter(|| {
                    let feature_opts =
                        FeatureOpts::new_behavior(ResolveBehavior::V2, has_dev_units);
