            ...
            for &mode in &[
                ...
                for (compile_kind, layout) in &layouts {
                    ...
                    for file_type in file_types {
                        ...
                        let hashed_name = format!("{}-*", target.crate_name());
                        let unhashed_name = target.crate_name();
                        // println!("hashed={:?}", file_type.filename(&hashed_name));
                        // println!("unhashed={:?}", file_type.filename(&unhashed_name));
                        rm_rf_glob(&dir.join(file_type.filename(&hashed_name)), config)?;
                        rm_rf(&dir.join(file_type.filename(&unhashed_name)), config)?;
                        // Remove dep-info file generated by rustc. It is not tracked in
                        // file_types. It does not have a prefix.
                        rm_rf_glob(&dir.join(hashed_name).with_extension("d"), config)?;
                        rm_rf(&dir.join(target.crate_name()).with_extension("d"), config)?;

                        // Remove the uplifted copy.
                        let uplift_target_name = if target.allows_dashes() {
                            // Some targets (like bins) allow dashes in the filename.
                            target.name().to_string()
                        } else {
                            target.crate_name()
                        };
                        let uplifted_path =
                            uplift_dir.join(file_type.filename(&uplift_target_name));
