...
pub fn fix_edition(
    ...
    let all_at_latest = |packages: &[&Package]| -> CargoResult<bool> {
        if packages
            .iter()
            .all(|p| p.manifest().edition() == Edition::Edition2024)
        {
            gctx.shell().status(
                "crater-edition-check",
                &format!("all packages now at latest edition"),
            )?;
            Ok(true)
        } else {
            Ok(false)
    ...
    match fix_edition {
        ...
        FixEdition::End {
            ...
            while !all_at_latest(&opts.compile_opts.spec.get_packages(&ws)?)? {
                ...
                // Do the edition fix.
                opts.edition = Some(EditionFixMode::NextRelative);
                ...
                let packages = opts.compile_opts.spec.get_packages(&ws)?;
                replace_edition(&packages)?;
                ws = original_ws.reload(gctx)?;
    ...
    eprintln!("crater-edition-check: all finished");
    Ok(())
}

/// Modifies the `edition` value of the given packages to the new edition.
fn replace_edition(packages: &[&Package]) -> CargoResult<()> {
    for package in packages {
        ...
        let current_edition = package.manifest().edition();
        let next_edition = current_edition.saturating_next();
        eprintln!(
            "crater-edition-check: {package} updating edition from {current_edition} to {next_edition}"
        );

        if let Some(package) = root.get_mut("package").and_then(|t| t.as_table_like_mut()) {
            package.insert(
                "edition",
                Item::Value(Value::String(Formatted::new(next_edition.to_string()))),
            );
        }
