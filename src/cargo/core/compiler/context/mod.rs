...
pub struct Context<'a, 'cfg> {
    ...
    pub build_explicit_deps: HashMap<Rc<Unit>, BuildDeps>,
    /// Fingerprints used to detect if a unit is out-of-date.
    pub fingerprints: HashMap<Rc<Unit>, Arc<Fingerprint>>,
    ...
    pub compiled: HashSet<Rc<Unit>>,
    /// Linking information for each `Unit`.
    /// See `build_map` for details.
    pub build_scripts: HashMap<Rc<Unit>, Arc<BuildScripts>>,
    ...
    rmeta_required: HashSet<Rc<Unit>>,
    ...
    /// process).
    pub rustc_clients: HashMap<Rc<Unit>, Client>,
}
...
impl<'a, 'cfg> Context<'a, 'cfg> {
    ...
    pub fn find_build_script_unit<'u>(&'u self, unit: &'u Unit) -> Option<&'u Unit> {
        // TODO: Consider returning Rc<Unit> to avoid weird lifetime annotation?
        // Or take &Rc<Unit> as parameter, and return &Rc<Unit>
        ...
        for (key, deps) in self.unit_dependencies.iter() {
            for dep in deps {
                if self.only_requires_rmeta(key, &dep.unit) {
                    self.rmeta_required.insert(Rc::clone(&dep.unit));
                }
            }
        }
    }
