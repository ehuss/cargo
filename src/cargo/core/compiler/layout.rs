...
//!     # scripts and examples. Almost every artifact should have a metadata
//!     # hash added to its filename to prevent collisions. One notable
//!     # exception is dynamic libraries.
//!     deps/
//!
//!     # All final artifacts are linked into this directory from `deps`.
//!     debug/  # or release/
//!
//!         # Symlink for backwards compatibility with older versions of Cargo.
//!         ../deps
//!
...
use crate::util::{paths, CargoResult, FileLock};
...
pub struct Layout {
    ...
    dest: PathBuf,
    /// The directory with rustc artifacts: `$root/deps`
    deps: PathBuf,
    /// The symlink to `deps`: '$root/debug/deps` (or `release`).
    /// This is for backwards compatibility with older versions of Cargo.
    deps_link: PathBuf,
    /// The directory for build scripts: `$root/build`
    build: PathBuf,
    /// The directory for incremental files: `$root/incremental`
    incremental: PathBuf,
    /// The directory for fingerprints: `$root/.fingerprint`
    fingerprint: PathBuf,
    /// The directory for examples: `$root/debug/examples` (or `release`).
    examples: PathBuf,
...
impl Layout {
    ...
    pub fn new(ws: &Workspace<'_>, triple: Option<&str>, dest: &str) -> CargoResult<Layout> {
        ...
        // compile.
        let lock = root.open_rw(".cargo-lock", ws.config(), "build directory")?;
        let root = root.into_path_unlocked();
        let root_dest = root.join(dest);

        Ok(Layout {
            deps: root.join("deps"),
            deps_link: root_dest.join("deps"),
            build: root.join("build"),
            incremental: root.join("incremental"),
            fingerprint: root.join(".fingerprint"),
            examples: root_dest.join("examples"),
            doc: root.join("doc"),
            dest: root_dest,
            root,
            _lock: lock,
    ...
    pub fn prepare(&mut self) -> CargoResult<()> {
        ...
        let dolink = match self.deps_link.symlink_metadata() {
            Ok(metadata) => {
                if metadata.is_dir() {
                    // Older versions of Cargo used a separate directory for
                    // release and debug.
                    paths::remove_dir_all(&self.deps_link)?;
                    true
                } else {
                    false
                }
        ...
        if dolink {
            paths::symlink("../deps", &self.deps_link)?;
        }
    ...
    pub fn deps(&self) -> &Path {
        ...
        // For example, this allows integration tests to use `current_exe`
        // to find executables in target/debug.
        &self.deps_link
    }
